...
export {
	...
	global ptr_threshold_idx: table[addr] of count &default=0 &write_expire=1 day &redef;
}
...
event zeek_done()
{
	for ( request_ip in ptr_queries ) {
		local sum = ptr_queries[request_ip]$noerror
		    + ptr_queries[request_ip]$nxdomain
		    + ptr_queries[request_ip]$refused
		    + ptr_queries[request_ip]$unknown;

		local out = fmt("ptr_queries: %s, %s, %s, sum: %s", request_ip,
		    ptr_queries[request_ip], ptr_queries[request_ip]$ptr_counts, sum);
